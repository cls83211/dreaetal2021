...
#number focals by treatment per month
...
ggsave(file="figure_2_color_no_raw.jpeg",ggplotfig2color2_no_raw,width=9,height=8.5,dpi=600)
```
...
Color Fig. S4 raw data (truncated to focals >5 min [>0.083] to reduce outliers on plot)
```{r}
Hexagonal_binning<-ggplot(minlengthmodeldata2, aes(x = age.months.cont, y = i.aggr.all/obs.time.hr,colour=X3.way))+geom_hex(bins=20)+scale_fill_gradientn(colours = c("#f5f4f4","grey","#242526"), name = "Frequency")+scale_colour_manual(values = c("#7570B3","#E60202","#FFC107"))+facet_wrap(~X3.way, labeller = labeller(X3.way = c("Dominant control" = "a","Subordinate control" = "b", "Dominant treated" = "c")))+labs(x=expression('Offspring age (months)'), y="Aggression initiated (per hour)",title = "")+ set_theme(axis.textcolor.y = "black",axis.textcolor.x = "black", axis.title.color = "black",base = theme_classic())+theme(plot.margin=unit(c(0,0,0,1.5),"mm"),strip.background = element_blank(),strip.text.x = element_text(hjust = -0.005,size = 15),text = element_text(size=15,family="Arial",lineheight=unit(1.5,"line")),legend.key.width=unit(1.5,"line"),legend.key.size = unit(1.5,"line"),legend.position ="right" )+guides(fill=guide_colorbar(reverse = TRUE),color=guide_legend(order=2,override.aes=list(fill="white",size = 1)),size = "legend")#+theme(strip.background = element_blank(), strip.text = element_blank())
Hexagonal_binning<-update_labels(Hexagonal_binning, list(colour = "Maternal treatment"))
Hexagonal_binning

ggsave(file="m.7_mindata_supp_hex_color_BW.jpeg",Hexagonal_binning,width=12,height=4.5,dpi=600)
```

B&W Fig. S4 raw data (truncated to focals >5 min [>0.083] to reduce outliers on plot)
```{r}
...
Color Fig.X raw data (All)
```{r}
Hexagonal_binning<-ggplot(modeldata2, aes(x = age.months.cont, y = i.aggr.all/obs.time.hr,colour=X3.way))+geom_hex(bins=20)+scale_fill_gradientn(colours = c("#f5f4f4","grey","#242526"), name = "Frequency")+scale_colour_manual(values = c("#7570B3","#E60202","#FFC107"))+facet_wrap(~X3.way)+labs(x=expression('Offspring age (months)'), y="Aggression initiated (per hour)",title = "")+ set_theme(axis.textcolor.y = "black",axis.textcolor.x = "black", axis.title.color = "black",base = theme_classic())+theme(plot.margin=unit(c(0,0,0,1.5),"mm"),strip.text.x = element_text(size = 15),text = element_text(size=15,family="Arial",lineheight=unit(1.5,"line")),legend.key.width=unit(1.5,"line"),legend.key.size = unit(1.5,"line"),legend.position ="right" )+guides(fill=guide_colorbar(reverse = TRUE),color=guide_legend(order=2,override.aes=list(fill="white",size = 1)),size = "legend")#+theme(strip.background = element_blank(), strip.text = element_blank())
Hexagonal_binning<-update_labels(Hexagonal_binning, list(colour = "Maternal treatment"))
Hexagonal_binning

ggsave(file="7_alldata_supp_hex_color_BW.jpeg",Hexagonal_binning,width=12,height=4.5,dpi=600)
```

B&W Fig.X raw data (all)
```{r}
Hexagonal_binning<-ggplot(modeldata2, aes(x = age.months.cont, y = i.aggr.all/obs.time.hr,colour="black"))+geom_hex(bins=20)+scale_fill_gradientn(colours = c("#f5f4f4","darkgrey","black"), name = "Frequency")+scale_colour_manual(values = "black")+facet_wrap(~X3.way)+labs(x=expression('Offspring age (months)'), y="Aggression initiated (per hour)",title = "")+ set_theme(axis.textcolor.y = "black",axis.textcolor.x = "black", axis.title.color = "black",base = theme_classic())+theme(plot.margin=unit(c(0,0,0,1.5),"mm"),strip.text.x = element_text(size = 15),text = element_text(size=15,family="Arial",lineheight=unit(1.5,"line")),legend.key.width=unit(1.5,"line"),legend.key.size = unit(1.5,"line"),legend.position ="right" )+guides(fill=guide_colorbar(reverse = TRUE),size = "legend", colour = "none")#+theme(strip.background = element_blank(), strip.text = element_blank())
Hexagonal_binning
ggsave(file="6_alldata_supp_hex_BW.jpeg",Hexagonal_binning,width=11,height=4.5,dpi=600)
```